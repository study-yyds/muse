...
jest.mock("../database/connection", () => ({
  ...
  schema: {
    ...
    user_api_keys: { id: "ak_id", user_id: "ak_uid", name: "ak_name", api_key_encrypted: "ak_enc", encryption_iv: "ak_iv", base_url: "ak_url", model_name: "ak_model", usage: "ak_usage", is_active: "ak_active" },
  },
}));
  ...
  describe("API Key CRUD", () => {
    ...
    it("listApiKeys 返回用户所有 Key", async () => {
      ...
      mockDb.orderBy.mockReturnValue(Promise.resolve([
        { id: "k1", name: "我的Key", model_name: "gpt-4", usage: "chat", is_active: true },
      ]));
    ...
    it("createApiKey 加密存储", async () => {
      ...
      const row = await service.createApiKey("user-1", {
        name: "测试Key", api_key: "sk-secret", base_url: "https://a.com/v1", model_name: "gpt-4", usage: "chat",
      });
    ...
    it("updateApiKey 不抛错", async () => {
      mockDb.where.mockReturnThis();
      await expect(service.updateApiKey("k1", "user-1", { name: "新名字" })).resolves.not.toThrow();
    });
